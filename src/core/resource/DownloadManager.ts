...
class SongDownloadStrategy implements DownloadStrategy {
  ...
  private async resolveUrl(): Promise<{ url: string; type: string }> {
    ...
    // QQ音乐歌曲使用 QQ音乐下载接口
    if (this.song.source === "qqmusic" || this.song.serverType === "qqmusic") {
      try {
        const songId = this.song.originalId || String(this.song.id);
        const qmResult: any = await import("@/api/qqmusic").then((m) =>
          m.qqMusicSongDownloadUrl(songId, levelName),
        );
        if (qmResult?.code === 200 || qmResult?.url) {
          const qmUrl = qmResult.data?.url || qmResult.url;
          if (qmUrl) {
            const extMatch = qmUrl.match(/\.([a-z0-9]+)(?:[?#]|$)/i);
            return { url: qmUrl, type: extMatch ? extMatch[1].toLowerCase() : "m4a" };
          }
        }
      } catch (e) {
        console.error("QQ音乐下载链接获取失败:", e);
      }
    }
