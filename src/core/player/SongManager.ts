...
class SongManager {
  ...
  public getOnlineUrl = async (id: number, isPc: boolean = false): Promise<AudioSource> => {
    ...
    let baseLevel: string = isPc ? "exhigh" : settingStore.songLevel;

    // Fuck AI Mode: 如果开启，且请求的 level 是 AI 音质，降级为 hires
    if (settingStore.disableAiAudio && AI_AUDIO_LEVELS.includes(baseLevel)) {
      baseLevel = "hires";
    }

    // 找到起始音质在降级顺序中的位置
    const startIndex = qualityFallbackOrder.indexOf(baseLevel);
    const levelsToTry = startIndex >= 0 ? qualityFallbackOrder.slice(startIndex) : [baseLevel];

    for (const level of levelsToTry) {
      // 跳过用户选择禁用的 AI 音质
      if (settingStore.disableAiAudio && AI_AUDIO_LEVELS.includes(level)) continue;

      try {
        const res = await songUrl(id, level as any);
        console.log(`🌐 [${id}] 尝试音质 ${level}:`, res?.data ? "有数据" : "无数据");

        // 兼容新旧接口的数据结构
        const songData = Array.isArray(res.data) ? res.data[0] : res.data?.[0];

        // 是否有播放地址
        if (!songData || !songData?.url) continue;
        ...
        // 若为试听且未开启试听播放，继续尝试下一个音质
        if (isTrial && !settingStore.playSongDemo) {
          console.log(`🔽 [${id}] ${level} 仅为试听，尝试降级`);
          continue;
        }
        ...
        // 检查本地缓存
        if (normalizedUrl && quality) {
          const cachedUrl = await this.checkLocalCache(id, quality, songData?.md5);
          if (cachedUrl) {
            return { id, url: cachedUrl, isTrial: false, quality };
          }
        }
        // 缓存对应音质音乐
        if (normalizedUrl) {
          this.triggerCacheDownload(id, normalizedUrl, quality);
        }

        console.log(`✅ [${id}] 使用音质 ${level}`);
   ...
   * @returns 在线播放信息
   */
  public getQQMusicUrl = async (song: SongType): Promise<AudioSource> => {
    try {
      // QQ音乐使用 songmid 或 id 作为标识
      const songId = song.originalId || String(song.id);
      const res: any = await import("@/api/qqmusic").then((m) =>
        m.qqMusicSongUrl(songId, "high"),
      );

      if (res?.data?.url || res?.url) {
        const url = res.data?.url || res.url;
        console.log(`🎵 QQ音乐 [${songId}] 获取到播放链接`);
        return {
          id: song.id,
          url,
          isUnlocked: false,
      ...
      console.warn(`QQ音乐 [${song.id}] 获取播放链接失败:`, e);
    }
  ...
  public getAudioSource = async (song: SongType, forceSource?: string): Promise<AudioSource> => {
    ...
    // QQ 音乐歌曲
    if (song.source === "qqmusic" || song.serverType === "qqmusic") {
      const qqMusicUrl = await this.getQQMusicUrl(song);
      if (qqMusicUrl.url) {
        return qqMusicUrl;
      }
      // QQ音乐失败，尝试解锁
      const canUnlockQQ = isElectron && song.type !== "radio" && settingStore.useSongUnlock;
      if (canUnlockQQ) {
